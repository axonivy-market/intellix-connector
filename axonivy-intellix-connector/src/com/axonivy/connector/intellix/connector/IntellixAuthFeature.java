...
public class IntellixAuthFeature implements Feature, ClientRequestFilter, ClientResponseFilter {
	...
	private static final String PROPERTY_CLIENT_NAME = "clientName";
			...
			if(getCookies() == null) {
				...
				if(StringUtils.isBlank(logonUrl) || logonUrl.trim().equalsIgnoreCase("AUTO")) {
					try {
						...
						String clientName = config.readMandatory(PROPERTY_CLIENT_NAME);
						logonUrl = Ivy.rest().client(clientName).resolveTemplate("host", host).path(ACCOUNT_LOGON_PATH).getUri().toString();
					} catch (Throwable t) {
